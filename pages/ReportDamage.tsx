...
import { resolveLocationFromGPS } from '../services/locationService.ts';
...
const ReportDamage = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          // Resolve a human-readable address using Gemini in the background
          try {
            const locationInfo = await resolveLocationFromGPS(coords.lat, coords.lng);
            setManualAddress(locationInfo.fullAddress);
          } catch {
            // Keep GPS-detected fallback — no disruption to user flow
          }
        },
        (error) => {
          console.error(error);
  ...
  const handleManualNext = async () => {
      if (pinnedLocation) {
          ...
          try {
            const info = await resolveLocationFromGPS(pinnedLocation.lat, pinnedLocation.lng);
            setManualAddress(info.fullAddress);
          } catch {}
      }
  };
  ...
  return (
    <div className="relative min-h-screen pb-20 md:pb-8 pt-6 px-4 max-w-2xl mx-auto flex flex-col">
      ...
      {/* Header & Stepper */}
      <div className="mb-8">
        <h1 className="text-3xl md:text-4xl font-display font-bold text-rastha-primary dark:text-white mb-6 tracking-tight">{t('reportPageTitle')}</h1>
          ...
          {!showManualInput ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm p-8 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm min-h-[400px]">
                ...
                <div className="max-w-xs">
                    <h2 className="text-2xl font-bold mb-2 dark:text-white font-display">{t('whereIsIssue')}</h2>
      ...
      {step === Step.UPLOAD && (
        <div className="flex-1 flex flex-col space-y-6 animate-fade-in pb-10">
             ...
             <div className="flex items-center gap-3 text-sm">
                ...
                <div>
                    ...
                    <p className="font-medium dark:text-gray-200 truncate max-w-[180px] sm:max-w-xs">
                        {manualAddress || (location ? `${location.lat.toFixed(4)}, ${location.lng.toFixed(4)}` : "GPS Position")}
                    </p>
                </div>
             </div>
          ...
          <div className="flex-1 bg-gray-50 dark:bg-slate-900/50 border-2 border-dashed border-gray-300 dark:border-slate-700 rounded-3xl relative overflow-hidden transition-all group hover:border-rastha-primary/50 dark:hover:border-blue-500/50 flex flex-col justify-center min-h-[250px]">
            ...
            />
            {imageError && (
              <div className="mt-3 flex items-start gap-2 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3">
          ...
          <div className="space-y-3">
              <label className="text-sm font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  ...
                  <span className="ml-auto text-xs font-normal text-gray-400">{description.length}/300</span>
              </label>
              <textarea
                  value={description}
                  onChange={(e) => setDescription(e.target.value.slice(0, 300))}
                  placeholder="e.g. Deep pothole in center lane, Water pipe burst..."
                  className={`w-full p-4 rounded-2xl bg-white dark:bg-slate-900 border focus:ring-2 focus:ring-rastha-primary outline-none text-sm min-h-[100px] resize-none text-gray-800 dark:text-gray-200 transition-colors ${description.trim() ? 'border-green-400 dark:border-green-600' : 'border-gray-200 dark:border-slate-700'}`}
              />
              {!description.trim() && (
                  <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
                      <AlertTriangle size={12} /> A brief description is required to submit your complaint.
                  </p>
              )}
          </div>
            ...
            <div className="text-left bg-gray-50 dark:bg-slate-950/50 p-5 rounded-2xl text-sm space-y-4 border border-gray-100 dark:border-slate-800">
               <div className="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-slate-800">
                 ...
                 <div className="flex items-center gap-2">
                   <span className="text-xs text-gray-400 dark:text-gray-500 font-mono">{aiResult.severityScore}/10</span>
                   <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${aiResult.severity === 'High' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : aiResult.severity === 'Medium' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-gray-200 text-gray-700 dark:bg-slate-800 dark:text-gray-300'}`}>
                      {aiResult.severity}
                   </span>
                 </div>
               </div>

               {/* ROAD TYPE — auto-detected from OSM */}
               {aiResult.roadType && (
                 <div className="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-slate-800">
                   <div>
                     ...
                     <p className="text-[10px] text-gray-400 mt-0.5">Auto-detected via OpenStreetMap</p>
                   </div>
               ...
               {/* AUTO-ROUTING RESULT */}
               {aiResult.autoRoutedDept && (
                 <div className="flex justify-between items-start pb-3 border-b border-gray-200 dark:border-slate-800">
                   <div>
                     <span className="text-gray-500 dark:text-gray-400 font-medium flex items-center gap-1.5">
                       <span className="w-2 h-2 rounded-full bg-blue-500 inline-block"></span>
                       Assigned Department
                     </span>
                     <p className="text-[10px] text-gray-400 mt-0.5">Automatically routed by our system</p>
                   </div>
                   <div className="text-right">
                     <span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 block">
                       {aiResult.autoRoutedDept}
                     </span>
                   </div>
                 </div>
                 ...
                 <div className="pb-3 border-b border-gray-200 dark:border-slate-800">
                   <div className="flex justify-between items-start mb-2">
                     ...
                     <span className="px-2 py-0.5 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded text-xs font-bold">
                       {aiResult.slaHours}h window
                     </span>
                   </div>
                   <p className="text-xs text-gray-600 dark:text-gray-300">
                     <span className="font-semibold">{aiResult.departments?.[0] || aiResult.autoRoutedDept}</span> must acknowledge by{' '}
                     <span className="font-bold text-orange-600 dark:text-orange-400">
                       {new Date(aiResult.slaDeadline).toLocaleString('en-IN', { 
                         day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'
                       })}
                     </span>
                   </p>
                   <p className="text-[10px] text-gray-400 mt-1">
                     Unacknowledged complaints auto-escalate to Commissioner
                   </p>
                 </div>
               )}
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-gray-100 dark:border-slate-700">
            {/* Header */}
            <div className="bg-rastha-primary px-6 py-5 flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center shrink-0">
                <Loader2 size={18} className="text-white animate-spin" />
              </div>
              <div>
                <p className="text-white font-bold text-base leading-tight">Complaint Submitted</p>
                <p className="text-white/70 text-xs mt-0.5">Rashtra AI Verification</p>
              </div>
            </div>

            {/* Warning Banner */}
            <div className="mx-5 mt-5 flex items-start gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-2xl px-4 py-3.5">
              <AlertTriangle size={18} className="text-amber-500 shrink-0 mt-0.5" />
              <p className="text-sm text-amber-800 dark:text-amber-300 leading-snug">
                AI verification may take{' '}
                <span className="font-bold text-amber-600 dark:text-amber-400">1–2 minutes</span>.
                Please keep this page open while your complaint is being processed.
              </p>
            </div>

            {/* Subtext */}
            <p className="text-xs text-center text-gray-400 dark:text-gray-500 mt-4 px-6">
              You'll be shown the result once verification is complete.
            </p>
            ...
            <div className="flex gap-3 px-5 py-5">
              <button
                ...
                className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-gray-300 font-semibold text-sm hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleConfirmedSubmit}
                className="flex-1 py-3 rounded-2xl bg-rastha-primary text-white font-bold text-sm flex items-center justify-center gap-2 hover:bg-[#082F4D] transition-colors shadow-lg shadow-rastha-primary/20"
              >
                <CheckCircle size={16} />
                Confirm &amp; Submit
              </button>
